...
pthread_mutex_t operationCountMutex = PTHREAD_MUTEX_INITIALIZER;
...
void *threadInput(void *data) {
    ...
    while(running){
        ...
        pthread_mutex_lock(&operationCountMutex);
        response.code = operationCount;
        operationCount++;
        pthread_mutex_unlock(&operationCountMutex);
...
void workAction(struct ClientMessage *message) {
    ...
    work = calloc(message->dataLen, sizeof(char));
    memcpy(work, message->data, message->dataLen);

    printf("%d, Client '%s' count all word: %d\n", work->taskId, message->clientName, work->allWord);
