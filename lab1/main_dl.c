...
int (*set_tab_size)(size_t);
void (*set_search_directory)(char* const);
void (*set_search_file_name)(char* const);
int (*set_tmp_file_name)(char* const);
int (*init)(void);
int (*find)(void);
int (*load_content_from_tmp_file)(void);
void (*remove_operation_item)(int);
void (*clear)(void);

int main(int argc, char *argv[]) {
    void* lib_handler = dlopen("/media/DATA/studia/sysopy/lab1/do_oddania/zadanie_3/libfind_operation.so", RTLD_LAZY);

    if(lib_handler == NULL){
        fputs("Can not load library.\n", stderr);
        exit(100);
    }

    check_file_exists = (int(*)(const char *))dlsym(lib_handler, "check_file_exists");
    set_tab_size = (int(*)(size_t))dlsym(lib_handler, "set_tab_size");
    set_search_directory = (void(*)(char* const))dlsym(lib_handler, "set_search_directory");
    set_search_file_name = (void (*)(char* const))dlsym(lib_handler, "set_search_file_name");
    set_tmp_file_name = (int (*)(char* const))dlsym(lib_handler, "set_tmp_file_name");
    init = (int (*)(void))dlsym(lib_handler, "init");
    find = (int (*)(void))dlsym(lib_handler, "find");
    load_content_from_tmp_file = (int (*)(void))dlsym(lib_handler, "load_content_from_tmp_file");
    remove_operation_item = (void (*)(int))dlsym(lib_handler, "remove_operation_item");
    clear = (void (*)(void))dlsym(lib_handler, "clear");
