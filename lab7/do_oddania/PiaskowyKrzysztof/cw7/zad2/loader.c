...
int main(int argc, char *argv[], char *env[]) {
    ...
    lineParamSM = getShareMemory(MEM_LINE_PARAM, lineParamsSize, 1, LOADER);
    ...
    lineSM = getShareMemory(MEM_LINE, lineSize, 2, LOADER);
    ...
    while(!endWork && ((argc > 3 && parcelCount > iteration))) {
        ...
        if(lineParams[0].mode) endWork = 1;
    ...
    lineParams = getLineParams(lineParamSM, lineParamsSize, LOADER);
    lineParams[0].loadersCountEndWord++;
    releaseLineParams(lineParams, lineParamsSize, LOADER);
