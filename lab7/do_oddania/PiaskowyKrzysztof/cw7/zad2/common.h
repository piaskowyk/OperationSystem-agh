...
int getShareMemory(const char * path, unsigned long size, int index, int type) {
    int ID;

    ID = shm_open(path, O_RDWR, S_IRWXU | S_IRWXG);
    if (ID == -1) {
        printErrorMessage("Error while getting line", type);
        exit(112);
    }
...
sem_t* getSemaphore(const char * path, int type) {
    sem_t* semId;

    semId = sem_open(path, O_WRONLY);
    if(semId == SEM_FAILED) {
        printErrorMessage("Error while creating semaphore", type);
        exit(112);
    }
