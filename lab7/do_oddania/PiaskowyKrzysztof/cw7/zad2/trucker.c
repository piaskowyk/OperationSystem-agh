...
int main(int argc, char *argv[], char *env[]) {
    ...
    lineSize = lineItemsCapacity * sizeof(struct Parcel);
    lineSM = setUpShareMemory(MEM_LINE, lineSize, 1, TRUCKER);

    //create shared memory for tracker args
    lineParamsSize = sizeof(struct LineParams);
    lineParamSM = setUpShareMemory(MEM_LINE_PARAM, lineParamsSize, 2, TRUCKER);
    
    //create semaphores
    semaphore = setUpSemaphore(SEM, TRUCKER);
    //save line parameters to shared memory
    blockSem(semaphore, TRUCKER);
    lineParams = getLineParams(lineParamSM, lineParamsSize, TRUCKER);
    lineParams[0].freePlaces = lineItemsCapacity;
    lineParams[0].freeWeight = lineWeightCapacity;
    lineParams[0].len = lineItemsCapacity;
    lineParams[0].mode = 0;
    releaseLineParams(lineParams, lineParamsSize, TRUCKER);

    //clearLine
    struct Parcel* lineClear = getLine(lineSM, lineSize, TRUCKER);
    ...
    while (1) {
        blockSem(semaphore, TRUCKER);
        line = getLine(lineSM, lineSize, TRUCKER);
        lineParams = getLineParams(lineParamSM, lineParamsSize, TRUCKER);
        ...
        releaseLine(line, lineSize, TRUCKER);
        releaseLineParams(lineParams, lineParamsSize, TRUCKER);
        releaseSem(semaphore, TRUCKER);
    ...
    //give inform about stop
    lineParams = getLineParams(lineParamSM, lineParamsSize, TRUCKER);
    lineParams[0].mode = 1;
    //wait until loaders close semaphores
    while (lineParams[0].loadersCount + 1 != lineParams[0].loadersCountEndWord) {
        sleep(1);
    }
    
    if(sem_unlink(SEM) == -1) {
        printErrorMessage("Error while remove semaphore", LOADER);
        exit(130);
    }

    if (shm_unlink(MEM_LINE) == -1) {
        fprintf(stderr, "\033[1;32mTrucker:\033[0m Error while removing shared memory (1).\n");
        exit(114);
    }

    if (shm_unlink(MEM_LINE_PARAM) == -1) {
